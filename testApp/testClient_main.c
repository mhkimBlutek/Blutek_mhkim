...
int main(int argc, char* argv[])
{
    ...
    serv_sock=socket(PF_INET, SOCK_STREAM, 0); //PF_INET = IPv4 프로토콜 체계
    memset(&serv_addr, 0, sizeof(serv_addr));
    serv_addr.sin_family=AF_INET;
    serv_addr.sin_addr.s_addr=inet_addr(argv[1]);   // 통신할 Server IP
    serv_addr.sin_port=htons(atoi(argv[2]));        // 통신할 Server Port
    ...
    receive_data rvcDT;
    rvcDT.message=message;
    rvcDT.serv_sock=&serv_sock;

    for(t=0;t<2;t++)
    {
        if(t==0) pthread_create(&p_thread[t], NULL, send_function, (void*)&serv_sock);
        else if(t==1) pthread_create(&p_thread[t], NULL, receive_function, (void*)&rvcDT);
    }
